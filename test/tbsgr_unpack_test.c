...
int main(void) {
	...
	int n = tb_sgr_unpack(codes, TB_BOLD|TB_ITALIC|TB_RED);
	...
	n = tb_sgr_unpack(codes, TB_BG|TB_GREEN);
	assert(n == 1);
	...
	// TB_BRIGHT attribute:
	n = tb_sgr_unpack(codes, TB_BRIGHT|TB_YELLOW);
	assert(n == 1);
	...
	n = tb_sgr_unpack(codes, TB_ITALIC|TB_UNDERLINE|TB_BLINK|TB_CYAN|TB_BG);
	assert(n == 4);
	...
	// from scratch:
	n = tb_sgr_unpack(codes, TB_RESET|TB_FAINT|TB_MAGENTA);
	assert(n == 3);
	...
	n = tb_sgr_unpack(codes, TB_DEFAULT);
	assert(n == 1);
	assert(codes[0] == 39);
	...
	n = tb_sgr_unpack(codes, TB_DEFAULT|TB_BG);
	assert(n == 1);
	...
	n = tb_sgr_unpack(codes, TB_UNDERLINE|TB_BLINK|TB_DEFAULT);
	assert(n == 3);
	...
	n = tb_sgr_unpack(codes, TB_ITALIC|TB_216|172);
	assert(n == 4);
	...
	n = tb_sgr_unpack(codes, TB_UNDERLINE|TB_BG|TB_GREYSCALE|10);
	assert(n == 4);
