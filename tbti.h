 ...
 */
typedef struct tb_term {
    tb_terminfo info;             // terminfo description
 ...
 */
#define TB_OK                 0x00
#define TB_ERR_TERM_NOT_SET   0x01
#define TB_ERR_FILE_NOT_FOUND 0x02
#define TB_ERR_FILE_INVALID   0x03

/*
 * Read the terminfo database and set up the tb_term structures for the given
 * terminal name, or the TERM environment variable when termname is NULL.
 *
 * Simplest invocation uses TERM and standard output:
 *     tb_term *term = tb_setupterm(NULL, 1, NULL);
 *
 * Returns a pointer to a newly allocated tb_term struct on success, or NULL
 * when an error occurs. Passing an int pointer in the err argument gives more
 * info on what kind of error occured:
 *
 *     TB_ERR_TERM_NOT_SET    no term name given and TERM not set.
 *     TB_ERR_FILE_NOT_FOUND  terminfo file not found or could not be read.
 *     TB_ERR_FILE_INVALID    file was found but is not a terminfo file.
 *
 ...
 *
 * The strings returned from tb_getstr() are owned by the tb_term struct and
 * should not be modified or freed. All strings are invalid after tb_freeterm()
