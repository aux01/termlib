...
int main(void) {
	...
	// The sgr struct type has a set of attribute bitflags, a
	...
	n = sgr_unpack(codes, (struct sgr){.at=SGR_BOLD|SGR_BG, .bg=SGR_GREEN});
	assert(n == 2);
	assert(codes[0] == 1);  // bold text
	...
	n = sgr_unpack(codes, (struct sgr){.at=SGR_FG16, .fg=SGR_YELLOW});
	assert(n == 1);
	assert(codes[0] == 93); // bright yellow foreground

	// Apply all typographic and background attrs without changing color:
	n = sgr_unpack(codes, (struct sgr){SGR_BOLD|SGR_FAINT|SGR_ITALIC|
	...
	sgr = (struct sgr){SGR_RESET|SGR_FAINT|SGR_FG, SGR_MAGENTA};
	...
	sgr = (struct sgr){SGR_NEGATE|SGR_BOLD|SGR_REVERSE|SGR_FG};
	n = sgr_unpack(codes, sgr);
	...
	n = sgr_unpack(codes, (struct sgr){SGR_FG, SGR_DEFAULT});
	assert(n == 1);
	assert(codes[0] == 39);

	// Same as above but apply the default foreground and background color:
	sgr = (struct sgr){SGR_FG|SGR_BG, SGR_DEFAULT, SGR_DEFAULT};
	n = sgr_unpack(codes, sgr);
	...
	sgr = (struct sgr){SGR_UNDERLINE|SGR_BLINK|SGR_FG, SGR_DEFAULT};
	n = sgr_unpack(codes, sgr);
	assert(n == 3);
	...
	n = sgr_unpack(codes, (struct sgr){SGR_UNDERLINE|SGR_BG24, 0, 10});
	assert(n == 4);
