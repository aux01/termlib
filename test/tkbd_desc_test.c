...
int main(void)
{
	...
	char buf[64];
	struct tkbd_seq seq = { .type = TKBD_KEY, .key = TKBD_KEY_A };

	int n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	assert(strcmp("A", buf) == 0);
	assert(n == (int)strlen(buf));

	seq.mod = TKBD_MOD_CTRL;
	n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	assert(strcmp("Ctrl+A", buf) == 0);
	assert(n == (int)strlen(buf));

	seq.mod |= TKBD_MOD_SHIFT|TKBD_MOD_ALT;
	n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	assert(strcmp("Ctrl+Shift+Alt+A", buf) == 0);
	assert(n == (int)strlen(buf));

	seq.key = TKBD_KEY_F12;
	n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	assert(strcmp("Ctrl+Shift+Alt+F12", buf) == 0);
	assert(n == (int)strlen(buf));

	seq.key = TKBD_KEY_F20;
	n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	assert(strcmp("Ctrl+Shift+Alt+F20", buf) == 0);
	assert(n == (int)strlen(buf));

	seq.key = TKBD_KEY_PGUP;
	n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	...
	// returns 0 when not a key sequence
	seq.type = TKBD_MOUSE;
	n = tkbd_desc(buf, sizeof(buf), &seq);
	printf("n=%d, buf=%s\n", n, buf);
	assert(n == 0);
